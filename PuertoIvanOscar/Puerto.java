...
public class Puerto
{
    ...
    public float liquidarAlquilerAmarre (int posicionAmarre)
    {
        ...
        if(posicionAmarre >= 0 && posicionAmarre <= amarre.length && amarre[posicionAmarre] != null ) {           //Compruebo si es un valor correcto, que esté entre 0 y 3 ( para el usuario)
    ...
    public int posicionPrimerAmarre ()
    {
        ...
        int i = 0;
        while(buscador && amarre.length > i){ 
             if(amarre[i] == null){
                 primerAmarreLibre = i;
                 buscador = false;
            }
    ...
    public boolean hayAmarresLibres ()
    {
        ...
        do{
            if(amarre[i] == null){
             ...
             i++;
        }while(i<amarre.length && !amarreLibre);
        return amarreLibre;
    }
}